...
/***/ function(module, exports) {
	...
	module.exports = {
		...
		"experience": {
			...
			"works": [
				{
					"name": "腾讯 微信事业群 开放平台（实习）",
					"start": "2016.07.01",
					"end": "至今",
					"description": [
						"参与跟进微信公众平台和开放平台的需求，使用过 `React` 开发内部系统",
						"设计各类实验探究浏览器渲染机制和缓存策略，总结前端最佳实践",
						"研究 `Google Accelerated Mobile Pages Project`，尝试使用其中的一些规范和方法优化性能，提高用户体验"
					]
				},
				{
					"name": "阿里巴巴 UC移动事业群 （实习）",
					"start": "2015.07",
					"end": "2015.09",
					"description": [
				...
				},
				{
					...
					"description": [
						...
						"使用 `React` 进行组件化编程，`Express` 搭建后台， `WebPack` 加载模块并驱动自动化工作流"
					]
				},
				{
					"name": "中大日记",
					...
					"end": "",
					"description": [
						"提供中大学子查询图书馆借阅记录，学杂费，校园卡消费等信息的服务",
						"基于 `Node.js` 通过学号和密码爬取相关信息，利用 `Promise` 优化代码结构"
					]
				}
			]
		},
		"skills": {
			...
			"description": [
				...
				"使用 `AngularJS` / `Ionic` 编写过简单的 WebApp，了解 `React` 及前端 `MV*` 模型",
				"了解 `HTTP`、 `TCP/IP`、 `WebSocket`、 `OAuth2.0` 等协议，具备基本的 Web 安全知识（`XSS` / `CSRF` 等）",
				"了解 `Three.js`，对数字图像处理感兴趣，能实现较复杂的3D图形程序",
